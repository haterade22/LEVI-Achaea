...
-- Trigger for swarm devour ready (kill reset or 40s cooldown expired)
-- Matches: "With a chorus of chittering mandibles..." OR "You may command a swarm to devour..."
if exists("Pariah Swarm Devour Ready", "trigger") == 0 then
	permRegexTrigger("Pariah Swarm Devour Ready", "", {
		[[^(With a chorus of chittering mandibles, the swarm rips away from the corpse of|You may command a swarm to devour another foe\.)]]
	}, [[ataxiaBasher_swarmDevourReady()]])
end
