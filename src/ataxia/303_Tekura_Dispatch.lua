...
tekura.state = {
  preferScythe = false,
  lastAttackTime = 0,
  useKaiSurge = true,  -- KAI Surge mounted opponents before leg break (costs 31 kai)
}
...
tekura.PHASES = {
  TORSO_PREP = 1,      -- SDK HKP HKP to prep torso (~86%)
  LEG_PREP = 2,        -- SNK HFP HFP to prep legs (~86%)
  ARM_BREAK = 3,       -- MNK + SPP + SPP to break an arm (needed for WRT)
  TORSO_BREAK = 4,     -- Break torso when all prepped, switch to HRS
  DOUBLE_BREAK = 5,    -- WRT arm + HFP legs, prones + breaks legs, switch to BRS
  KILL = 6,            -- BBT until dead (in Bear stance)
  SCYTHE = 7,          -- Alternative: Telepathy kill
}
...
-- Check if a specific arm is prepped (one punch away from breaking)
function tekura.dispatch.checkArmPrepped(arm)
  local sppDamage = 14  -- SPP does 14% to arms
  local limbName = arm .. " arm"
  local armDmg = tekura.dispatch.getLimbDamage(limbName)
  return armDmg + sppDamage >= tekura.config.breakThreshold and armDmg < tekura.config.breakThreshold
end

-- Check if a specific arm is broken
function tekura.dispatch.checkArmBroken(arm)
  local limbName = arm .. " arm"
  local armDmg = tekura.dispatch.getLimbDamage(limbName)
  return armDmg >= tekura.config.breakThreshold
end

-- Check if any arm is broken
function tekura.dispatch.checkAnyArmBroken()
  return tekura.dispatch.checkArmBroken("left") or tekura.dispatch.checkArmBroken("right")
end

-- Get the broken arm (for WRT)
function tekura.dispatch.getBrokenArm()
  if tekura.dispatch.checkArmBroken("left") then
    return "left"
  elseif tekura.dispatch.checkArmBroken("right") then
    return "right"
  end
  return nil
end

-- Get the arm to break (prefer lower damage, avoid parried)
function tekura.dispatch.getArmToBreak()
  local parried = tekura.dispatch.getParried()
  local laDmg = tekura.dispatch.getLimbDamage("left arm")
  local raDmg = tekura.dispatch.getLimbDamage("right arm")

  -- If one is already broken, don't break it again
  if tekura.dispatch.checkArmBroken("left") then
    return "right"
  elseif tekura.dispatch.checkArmBroken("right") then
    return "left"
  ...
  -- Prefer the arm with more damage (closer to breaking)
  -- But avoid parried arm
  ...
  -- Default to the one with more damage
  return laDmg >= raDmg and "left" or "right"
end
...
-- Check if target is mounted (SWK doesn't work on mounted)
function tekura.dispatch.checkMounted()
  tAffs = tAffs or {}
  return tAffs.mounted or false
end
...
function tekura.dispatch.getPhase()
  ...
  -- DOUBLE_BREAK: Torso is broken, ARM is broken, both legs prepped
  -- WRT broken arm will prone AND break legs
  if tekura.dispatch.checkTorsoBroken()
     and tekura.dispatch.checkAnyArmBroken()
     and tekura.dispatch.checkBothLegsPrepped() then
    return tekura.PHASES.DOUBLE_BREAK
  end

  -- TORSO_BREAK: ALL are prepped + ARM is broken, break torso and go to Horse
  if tekura.dispatch.checkAllPrepped() and tekura.dispatch.checkAnyArmBroken() then
    return tekura.PHASES.TORSO_BREAK
  end

  -- ARM_BREAK: Torso and legs are prepped but no arm is broken yet
  -- Need to break an arm before we can use WRT
  if tekura.dispatch.checkAllPrepped() and not tekura.dispatch.checkAnyArmBroken() then
    return tekura.PHASES.ARM_BREAK
  end
...
function tekura.dispatch.buildTorsoPrepAttack()
  local parried = tekura.dispatch.getParried()
  local mounted = tekura.dispatch.checkMounted()

  -- If they're parrying torso, prep legs instead
  if parried == "torso" then
    if mounted then
      -- Can't SWK mounted, use SNK on legs
      return "combo " .. target .. " snk left hfp left hfp right"
    else
  ...
  -- If they're parrying a leg, use SWK to bypass (or SNK if mounted)
  if parried == "left leg" or parried == "right leg" then
    if mounted then
      -- Can't SWK mounted, use SNK on non-parried leg
      local otherLeg = parried == "left leg" and "right" or "left"
      return "combo " .. target .. " snk " .. otherLeg .. " hfp left hfp right"
    else
...
function tekura.dispatch.buildLegPrepAttack()
  ...
  -- If parrying a leg, use SWK to bypass (or SNK on other leg if mounted)
  if parried == "left leg" or parried == "right leg" then
    if mounted then
      -- Can't SWK mounted, use SNK on non-parried leg
      local otherLeg = parried == "left leg" and "right" or "left"
...
-- PHASE 3: Arm Break - MNK + SPP + SPP to break an arm (needed for WRT)
-- MNK = 25% arm damage, SPP = 14% arm damage = 53% per combo (2 combos to break)
function tekura.dispatch.buildArmBreakAttack()
  local armToBreak = tekura.dispatch.getArmToBreak()
  local parried = tekura.dispatch.getParried()
  local mounted = tekura.dispatch.checkMounted()

  -- If they're parrying the arm we want, switch to the other arm
  if parried == armToBreak .. " arm" then
    armToBreak = armToBreak == "left" and "right" or "left"
  end

  -- MNK (moon kick) = 25%, SPP (spear punch) = 14% each
  -- Full combo: mnk + spp + spp = 53% damage to arm
  return "combo " .. target .. " mnk " .. armToBreak .. " spp " .. armToBreak .. " spp " .. armToBreak
end

-- PHASE 4: Torso Break - SDK HKP HKP;HRS (break torso, switch to Horse)
...
-- PHASE 5: Double Break - WRT arm HFP left HFP right;BRS
-- This prones AND breaks both legs, then switches to Bear
function tekura.dispatch.buildDoubleBreakAttack()
  -- Use the BROKEN arm for WRT (not just any arm)
  local wrtArm = tekura.dispatch.getBrokenArm() or tekura.dispatch.getWrtArm()
...
-- PHASE 6: Kill - BBT until dead (in Bear stance)
...
-- PHASE 7: SCYTHE - Telepathy kill alternative
...
function tekura.dispatch.selectAttack()
  ...
  elseif phase == tekura.PHASES.ARM_BREAK then
    return tekura.dispatch.buildArmBreakAttack()
...
function tekura.dispatch.run()
  ...
  -- KAI Surge mounted opponents before DOUBLE_BREAK (costs 31 kai)
  -- This prevents them from remounting after leg break
  if tekura.state.useKaiSurge and tekura.dispatch.checkMounted()
     and phase == tekura.PHASES.DOUBLE_BREAK then
    cmd = cmd .. "unwield all;dismount;kai surge " .. target
    send("queue addclear free " .. cmd)
    cecho("\n<yellow>[TKD] KAI SURGE (dismount before leg break)")
    return
  end
...
function tekura.dispatch.status()
  ...
  cecho("\n<yellow>|   <white>L Arm: " .. prepStatus(laDmg) .. string.format("%5.1f%%", laDmg) .. "<reset> [<orange>" .. progressBar(laDmg) .. "<reset>] <grey>(WRT)")
  cecho("\n<yellow>|   <white>R Arm: " .. prepStatus(raDmg) .. string.format("%5.1f%%", raDmg) .. "<reset> [<orange>" .. progressBar(raDmg) .. "<reset>] <grey>(WRT)")
  ...
  cecho("  <white>Arm Broken: " .. (tekura.dispatch.checkAnyArmBroken() and "<green>YES" or "<red>NO"))
  cecho("\n<yellow>|   <white>Torso Broken: " .. (tekura.dispatch.checkTorsoBroken() and "<green>YES" or "<red>NO"))
  cecho("  <white>Mounted: " .. (tekura.dispatch.checkMounted() and "<red>YES" or "<grey>NO"))
  cecho("\n<yellow>+--------------------------------------------+")
  ...
  cecho("   <white>KAI Surge: " .. (tekura.state.useKaiSurge and "<green>ON" or "<grey>OFF"))
  cecho("\n<yellow>+--------------------------------------------+")
  ...
  cecho("\n<yellow>|   " .. (phase == 3 and "<white>" or "<grey>") .. "3. ARM_BREAK: MNK SPP SPP (break arm for WRT)")
  cecho("\n<yellow>|   " .. (phase == 4 and "<white>" or "<grey>") .. "4. TORSO_BREAK: SDK HKP HKP -> HRS")
  cecho("\n<yellow>|   " .. (phase == 5 and "<white>" or "<grey>") .. "5. DOUBLE_BREAK: WRT arm HFP HFP -> BRS")
  cecho("\n<yellow>|   " .. (phase == 6 and "<green>" or "<grey>") .. "6. KILL: BBT until dead (BRS)")
  cecho("\n<yellow>|   " .. (phase == 7 and "<magenta>" or "<grey>") .. "ALT: SCYTHE if head prepped + battered")
  cecho("\n<yellow>+============================================+\n")
end
...
-- Toggle KAI Surge mode
function tekura.dispatch.toggleKaiSurge()
  tekura.state.useKaiSurge = not tekura.state.useKaiSurge
  cecho("\n<yellow>[TKD] KAI Surge mode: " .. (tekura.state.useKaiSurge and "<green>ENABLED" or "<grey>DISABLED") .. " <grey>(costs 31 kai)<reset>")
end

-- Reset state for new target
function tekura.dispatch.reset()
  tekura.state.preferScythe = false
  tekura.state.useKaiSurge = true
  tekura.parry.clear()
...
function tkdsurge()
  tekura.dispatch.toggleKaiSurge()
end
