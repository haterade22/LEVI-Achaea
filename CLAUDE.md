...
**Run end:** `/run_end` fires on `"The Mnemosyne releases its hold, weaving N shimmering threads into your possession."` — but only after a **confirmation** (`onRunEndMaybe` waits ~2s for `"You just received message #N from Achaea."`), because that reward line also prints on a mid-run message re-read. The Mnemosyne boon flags (`bardWarmarch` / `bmShatteredStar` / `magiKkractle` / `magiHotSprings` / `mnemHammerAnvil` / `bmBladedReflexes` / `mnemSleuth` / `mnemRollHide` — Hammer and Anvil is class-agnostic: attacks bypass denizen shields, so `336_Mob_Shielded` skips the raze path AND the shield-swap while it's up; Bladed Reflexes makes the BM basher keep `SHIN AUGMENT <n>` up (spend `ataxiaBasher.bmAugmentAmount`, default 3 — a 1-shin augment dissipates in ~12ms per live log) — 20% DR while the `bodyaugment` defence holds; Sleuth arms the swarm module's fullsense-on-GO recon; Roll Hide arms the swarm panic tumble) are cleared only on the confirmed end (and reset on run start). `startRun` also self-recovers: its `onError` resets `run.active` if `/run_start` 500s or times out. All events auto-report; manual `mnem` overrides remain.
