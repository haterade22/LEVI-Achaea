...
**CHECK 2 -- a namespace FIELD called but never defined (v4.7.281).** `attempt to call field 'echo' (a nil value)`: **`ataxia.echo` exists nowhere in the tree** -- the helper is the global `ataxiaEcho`, built in a `_groups.yaml` inline script -- so **17 call sites across 4 files were dead**, including every echoing branch of `bash shinprobe` and **all nine echoes in `ataxiabars`**. The same sweep found `shaman.help()`, called unguarded by `sp help` and defined nowhere. Check 1 cannot see either: it reasons about GLOBALS defined by INACTIVE scripts, and these are table FIELDS defined by nothing (as was the v4.7.264 `zgui` family, an INDEX rather than a call). `check_missing_fields()` scans `.lua` AND `_groups.yaml` for `NS.field(...)` whose `NS.field` is assigned nowhere. Kept quiet enough to survive: **`mmp` is excluded** (a separate package, ~220 functions outside `src_new`, hundreds of false positives), **comments are stripped** (`-- Set ataxiaBasher.fleeTimeout (seconds)` otherwise reads as a call), and **the guard window is TWO LINES** rather than one, because the idiom here puts the test on its own line (`if ataxia and ataxia.decho then` / call / `end`) and per-line strictness flagged six safe sites. Findings report the REAL file line -- `split()` drops the header, and a finding you cannot jump to is one you will not act on. **A gate that cries wolf gets switched off, which is how a guard becomes decoration.**

**`tools/check_colours.py` -- a colour name is only valid in OUR palette (v4.7.287).**
`007_Custom_Colour_Table.lua` WHOLESALE REPLACES Mudlet's, so a name that is perfectly good in stock
Mudlet is absent here and makes the render throw (v4.7.136: `silver`). The script asserts every
colour name a named file chooses exists in that table, and that none is in the **reserved orange
family**. **It scans two things and the second is the point:** the `<tag>` / `fg("name")` forms a
RENDERER writes, AND the `colour = "name"` fields plus `*_COLOUR` table values an AGGREGATOR
...
- **Own denizens** (`ataxiaBasher.ownDenizens` / `bash mine`): pet/ally name keywords excluded from auto-learn and targeting without skipping the room. Matched by case-insensitive SUBSTRING, which cuts both ways: `ataxiaBasher.notOwnDenizens` / `bash notmine` (v4.7.169) exempts a REAL denizen whose name merely contains a pet's word and WINS over the keyword -- "a slope-backed hyena" was shielded by the `hyena` keyword seeded for the Infernal pet. In Mnemosyne the consequence is that the SWEEP WALKS AWAY FROM A LIVE MOB: `_roomHasDenizens`/`_denizenCount` (008_Explorer:97,108) filter own denizens too, so a room holding only the shadowed mob reads as CLEAR and the explorer navigates out, trailing an aggressive denizen (v4.7.169 called this a stall; corrected v4.7.170 -- it is silent, not stuck). Seeded by backfill, since existing saves already carry the bare keyword. The user's five MOUNTS are on the list too (v4.7.174) and are keyed on their FULL descriptive name — `lean grizzly bear`, never `bear` — for exactly the same reason: a bare creature noun would shadow half the bestiary. **Mounts are also excluded BY ID since v4.7.335** (`basher/013_Mounts.lua`, triggers 776/777 read "Your loyal mounts are:" and its rows; `ataxiaBasher_isMount(id)` drops them in `ataxia_RoomContents_Update` so they never reach `denizensHere`) — the id is exact where the name is not, so a WILD dire wolf or grizzly bear is still a target. Reading the listing also purges the mount's EXACT name from the saved target lists, and the keyword backfill purges once on load: v4.7.174 stopped them being attacked but left them sitting in `bash list`, which is what "not on the kill list" was actually about. `bash mounts` / `bash mounts clear`. **VAULT sets the mount (v4.7.339):** the trigger used to send `curing mount <descriptive name>` off the vault line and the game refused it ("That is not a valid mount that belongs to you"), so nothing was set and every later VAULT/SPUR/"come here" used the old configured one. `ataxiaBasher_vaultedOnto(name)` resolves the name to the learned ID (exact match -- "a grizzly bear" is not "a lean grizzly bear"), stores it as `ataxia.settings.user.mount` for `ataxia.getMount()`, sends `curing mount <id>` and saves; with no listing read it says so and changes nothing rather than repeating the refusal. **MOUNTED, WE MOUNTJUMP (v4.7.345, user-directed):** from the saddle the game refuses a leap outright ("You cannot do that while mounted.") and that refusal is a SILENT no-op in the escape ladder, so `ataxiaBasher_isMounted()`/`ataxiaBasher_mountVerb(fallback)` own a belief that every jump site consults. Nine lines teach it and seven already had triggers -- mounted: the two vault/climb lines (025/735), the mountjump landing "You pull back the reins on your mount and jump off to the <dir>." (783) and the refusal itself (784; renamed from 737/738 in v4.7.351 -- the folder already used those numbers); on foot: "You step down off of"/"You lose purchase on" (736, the second being thrown) and all three no-mount refusals (705, incl. "You have no mount on which to jump."). BOTH directions are taught by a refusal as well as a success, so a wrong belief costs one command and then it is right. Lives in `ataxiaTemp`, NOT `ataxiaBasher` -- `ataxiaBasher` is saved, and a saved `mounted = true` would outlive the reload that put us on our feet (the v4.7.336 `_busy` shape); unknown reads as on foot. `ataxiaBasher_jumpRefusedMounted` also RE-ISSUES the lost move as a mountjump -- the escape that queued the leap is waiting on an arrival that will never come -- but only a jump WE sent, from the room we sent it in, within 4s, and never one that was already a mountjump -- the full set of commands that earn the line is UNKNOWN, so bounding the recovery to a jump of our own makes an unknown sibling refusal cost the latch and nothing else. **TUMBLE is not one of them** (v4.7.346, user: "tumble goes through while on a mount, by the way"): v4.7.345 guessed the opposite and flagged the Roll Hide panic tumble as a livelock waiting to happen -- it is not, and the tumble sites need no conversion. The Bard's BACKFLIP is taken onto the mountjump path as a deliberate default, NOT a confirmed refusal (never observed); if it proves fine mounted, move the mounted check below the class branch and the Bard keeps its faster balance. `bash mounts` opens with which verb is live
...
**THE SWAP ASKS BEFORE IT ACTS (v4.7.323, user-directed).** Every `armour <profile>` sends `ii paragon` + `probe armour` (`ataxia.armour.refresh`), and plans from the answers with the pure `ataxia.armour.planSwap(slots, cur, known, inventory, capacity)`. The probe is a SNAPSHOT: the header opens it, embrasure lines fill it, it commits `PROBE_SETTLE` later, so an unlisted or `N: Empty.` embrasure reads EMPTY -- and only probes the module sent are read (`state.probing`), so probing another armour cannot corrupt the worn one's state. Plan rules: a slot already holding the right TYPE (`paragonKey`) is kept; a want we cannot supply (not in the inventory, not released by another embrasure this swap -- resolved to a fixed point) is reported `NEED` and its embrasure is NOT TOUCHED, never emptied; a profile is a list of TYPES, but **the insert names an ID** -- the one `ii paragon` showed for the paragon of that type we hold (v4.7.326: v4.7.323 sent the type word, and `insert icosagon` / `insert metalliferous` were refused, "That is not a valid paragon.", with both in the inventory -- the type word is the item's keyword only for one-word names like "a crucious paragon"; the pry had gone out, so embrasures were left EMPTY); with no inventory answer, the profile's own id, else `idForType`, else the bare word; all pries before all inserts (the old loop interleaved them, so a paragon could not move between embrasures); slots past the armour's capacity are skipped. `planReport` prints one line per embrasure, and `verifySwap` probes again `VERIFY_DELAY` (1.5s) later -- pry/insert answer instantly (v4.7.211 log) -- and reports `Verified` or the embrasure that did not take. **An unanswered refresh means NO swap (v4.7.326 review):** no pries, no inserts, traits only, and an echo -- a swap planned from an old picture pries blind, and a pry whose insert is refused is how v4.7.323 emptied embrasures (this replaced v4.7.323's "plan from the earlier picture" rule). `idForType(k, busy)` passes over ids still in an embrasure the swap KEEPS and picks the lowest by NUMBER. `armour probe [profile]` is the dry run; `armour scan` uses the same refresh, so it registers paragons IN the armour, which `ii paragon` never lists. Probed paragons keep the game's effect text (`config.paragonEffects`). Tests load the real script (`test_armour_swap_diff.lua`); the old test copied the planning logic, and a copy cannot catch a regression.
...
**`combo_boon`, THE NINTH `BoonInfo` FIELD (v4.7.322, schema re-verified 2026-09-18).** The tracker
added a boolean `combo_boon`; everything else still matched. The wording is known ONLY from the
tracker's `GET /boons/export`, where ten descriptions carry `Combo Boon?:        Yes` GLUED ON --
which screen prints it was then unproven -- **since PROVEN: CONTEMPLATE prints it** (Flameheart's and Elder Wisdom's blocks, v4.7.326, both "Yes"). "No" is still unseen -- Careless Whisperer's block has no combo line at all, so a non-combo boon may simply omit it.
...
is 2+ spaces after the colon -- column padding, never prose; matched exactly the corrupted strings
across ~1,100 descriptions), `_learnBoon` never stores one, `M._boonDbRepair` fixes stored ones at
...
active; `ataxiaTemp.boonsOwned` is never cleared so it is not used; gold otherwise, as in the summary) and
adds the list to the block's summary line; the offer screen's `M._echoBoonCallouts` (v4.7.326; it
replaced `_echoConflicts`) warns per offered boon, reading the pair from either side. The real
block's values sit at column 21, like the glued "Combo Boon?" lines -- and Flameheart's block (v4.7.326)
then showed `Combo Boon?:        Yes` outright, so that line IS CONTEMPLATE's. **Call-outs (v4.7.326,
user: "should called out if the boon can combo and echo"):** trigger `mnemosyne/093` highlights a
`Combo Boon?: Yes` / `Can echo: Yes` / `Maximum echoes: N` in place, and each contemplate block gets
ONE summary line (`M._calloutAdd` gathers combo/echo/conflicts; `_calloutFlush` prints them in that
order). **The aggregator is module-global, so it flushes at a BLOCK BOUNDARY** -- 093 also matches the
closing divider (`^-{3,}`) and the next block's `^Rarity:` -- because a contemplate chain sends the next
block 0.5s after the last closed; the 0.4s quiet timer is only the backstop, a flush kills it, and a
reload resets it. The offer screen's `M._echoBoonCallouts` gives each offered boon the same call-outs
from the catalogue. **`echoFloor`**: "Can echo: Yes" with no "Maximum echoes" line parses as 1, a
floor -- flagged so the offer screen says "can echo", never "(max 1)", and never allowed to replace a
...
**Putting Phoenix first exposed two latent traps in the augment block**, neither of which was style: it had no `shinSpent` check and it wrote `command = "shin augment "..` -- an **ASSIGNMENT** -- because nothing had ever preceded it. The phoenix was built and then silently destroyed, and the round went out with the augment alone. **A "one spender per round" rule enforced by position alone breaks the moment the positions change.** **Deluge affix** (v4.7.140, trigger 037 → `onDelugeSeen`, same shape): "All rooms are underwater" makes FLY impossible — `S._canFly()` gates the escape ladder's outdoor fly+hover (falls through to the grounded retreat / shield fallback) and the fly-kite entry, so neither wedges on a silently-rejected queued fly. **Dragged out of the sky** (v4.7.168, trigger `mnemosyne/050` → `S.onDraggedDown`): "A tentacle shoots up from the ground, wraps itself around you, and drags you back to earth." — a DENIZEN can pull us out of the air. Third way flight fails after Deluge and an eaten FLY, and the worst, because it is SILENT to the state machine: the hover keeps `S.flying` optimistically true until a flight line confirms (the guard that exists because stupidity eats queued commands), and after a drag that confirmation never comes — so the hover re-sends `fly` EVERY TICK while the tentacle yanks us back, holding us attack-GATED at crash HP until `RECOVER_MAX` expires. Latches `S.grounded` (honoured by `S._canFly` and hence `S._canHover`), corrects the flight state, and converts an in-progress hover into the grounded retreat. **Per-RIPPLE** — `S.onRipple` clears it. **BOILING LAVA -- the only unconditional "leave now"** (v4.7.254, hardened v4.7.256, trigger `mnemosyne/064` → `M.onLava`/`M.roomLava`): `You splash into boiling lava!` / `You continue to struggle in the boiling grasp of the lava as it eats away at your body.` -- **5,890 UNBLOCKABLE per tick** against a 10,939 pool, i.e. 54% of the pool, two ticks is a death, and unblockable means no shield, barrier or resistance helps. Every other hazard here can be fought through (Ablaze is ~1,200 and only gates the hover), so this is **the exception to the validated-route rule**: the escape ladder refuses unvalidated exits by user decision, but staying costs half the pool per tick, so ANY door beats the floor. **Exit order changed in v4.7.297, user-directed** (live log: exits north and west, the old order picked north -- the room already cleared -- while west led into the unswept grid): an exit `MAP.unexploredExits` has never walked now outranks the room we just left. **And again in v4.7.318, from a death: ANY forward door outranks back, walked or not.** The entry damage is SUNK -- "once we enter the room, we already take the damage" -- so retreating buys nothing against it and guarantees paying it again, because whatever drew us in (a denizen standing in the lava, an exit beyond it) is still there and the sweep or patrol routes straight back. In a well-fought room every door has a walked edge, nothing is "unexplored", and v4.7.297's pass found nothing; the death log shows enter / 5,890 / retreat / re-enter for the phantom grizzly / 5,890 / ... The order is now: any UNEXPLORED planar exit (sorted, excluding known lava) → any FORWARD planar exit not known lava (sorted, i.e. anything but the way we came) → back the way we came (last resort: provably not lava, most likely to bring us straight back) → the blind fallback → `down`. **`_nextPatrolStep` had NO lava filter at all until v4.7.318** -- the sweep and backtrack learned it in v4.7.256, the patrol's only test was `planarStep`, so once the grid was swept it round-robined the lava room forever; it now skips a known lava TARGET and a known lava first-step EDGE. The target check is not redundant: a prose-sourced exit (`0`) is unresolvable by `_exitTarget`, and `lavaEdges` only knows the door we splashed through, so a lava room reached by any other door is invisible to the edge check. **GLANCE RECON (v4.7.318, user-directed):** the sweep glances every door it has never walked before stepping (`M._glanceWanted`/`_glanceStart`, `GLANCE_TIMEOUT` 1s) -- GLANCE is free and prints the neighbour's description and exits line. It never refuses a room (the rule is enter and pass through); it PLANS: "Molten lava bubbles and churns." inside a glance (trigger `mnemosyne/090`) makes `_glanceResolve` record `ataxiaTemp.mnemLavaPlan = {room, dir, fwd}` from the glanced exits, and `onLava` fires `fwd` on the splash instead of deriving a door under fire. The glanced exits reach 008 because 005's one-shot glance token -- which existed to DISCARD a neighbour's exits line (v4.7.262) -- now also publishes it (`M._onGlanceExits`, guarded). Backtracks over walked edges get no glance. `ataxia.settings.reporting.glance = false` turns it off. `MAP.parseExitsLine` returns a SET keyed by long direction, not a list -- iterate with `pairs`. **Four review hardenings, same version:** `onLavaSeen` only counts a description line while the header token is armed for the pending direction (our OWN room's description prints on arrival/LOOK/ql and would otherwise mark the neighbour); the resolve re-ticks only if we are still in the room we glanced from (`_scheduleTick` is last-call-wins and would otherwise kill an arrival settle if something moved us mid-glance); the tick gate force-resolves a glance older than 3x `GLANCE_TIMEOUT` (a lost timer must not hold the sweep forever); and `_lavaExit`'s forward AND fallback passes use `edgeIsLava`, not `lavaRooms[_exitTarget]`, because a prose exit's destination is unresolvable and only the EDGE remembers a neighbour we splashed into from here. **v4.7.319 (the full multi-agent review of v4.7.318, probe-confirmed):** the patrol filter had created a LIVELOCK -- a lava room with its own unwalked exit was still a backtrack TARGET, refused only at the first step, so the sweep and patrol traded steps forever; the backtrack now never targets a lava room and both backtrack and patrol check the WHOLE route (`M._pathIsLavaFree`). `onLava`'s plan was gated on `first` (episode timing) instead of `entry` (the splash), so a second lava room entered within the episode gap skipped its plan and inherited the first room's REMEMBERED door -- the way we came; now gated on `entry`, and the remembered door is scoped to the episode's anchor room. Both the planned and remembered doors are `edgeIsLava`-checked at the splash (the plan cannot know at glance time). 005's send capture no longer lets `glance`/`squint`/`observe`/`look` set the last move direction (a look moved us: phantom walked edges). A glance pending for a room we LEFT is dropped at the gate (v4.7.318's settle fix otherwise stalled 30s). No glance while BLIND. `mnem explore glance [on|off]`; `mnem explore status`/`why` show the glance state. An unexplored room CAN be lava -- that is what unexplored means -- but the worst case is one more tick of damage before the next struggle line re-picks a door, where refusing to explore never recovers the sweep. Holds the attack dispatcher, resets any swarm tactic, re-sends every tick (an eaten move must be retried), and on no known exit sends `ql` and says **MOVE MANUALLY**. Both lines are used: the splash is entry, the struggle is the tick, and the tick fires without an entry line when we were already standing in it. **v4.7.256 was a DEATH**: marking the room caught one code path and three others still led back in -- the sweep BACKTRACK (once the room was walked its exits stopped counting as unexplored, but the unexplored exit BEYOND it made lava the shortest path, and `MAP.path` has no hazard filter), `S._backDir` (the room we came from is normally the safest square) and `S._panicDir`. **Remember the EDGE, not just the room**: room-keyed marking is unusable from the room next door, because `_exitTarget` returns nil when gmcp has not filled a destination id -- exactly the case for an unvisited neighbour. `M.explore.lavaEdges[from][dir]`, recorded at the moment we splash, needs no id from anyone; shared predicates `M.roomIsLava`/`M.edgeIsLava` feed all four consumers. The backtrack checks only the FIRST step (every step is re-decided on arrival, so a route we never enter is one we never traverse), and `_backDir` returning nil drops the ladder to shield-in-place -- bad, not fatal. **"We walked through it" is not the same fact as "it is survivable."** Two guards found while testing: a stale `explore.fromRoom` (current only after a sweep step, not a tumble or chase) would mark an edge out of a non-adjacent room and refuse a good exit forever, so recording requires `from ~= cur`; and both exit scans are SORTED, because an unordered choice makes the log unreadable and made the guards untestable.
...
seconds." User: "if we have this boon, we should use flourish every 15 seconds." AB Flourish
(Bladedance): `BLADE FLOURISH <target>`, **2.10s of BALANCE**, adventurers-only, "may not inflict
any harm" -- its whole base purpose is to "advance two positions through the bladedance
immediately (front to back, side to front, back to side)". Three AB facts, three gates: BALANCE
means it **REPLACES the flick** (the Songstep rule above; the battlerage still rides), adventurers-
only means the **boon is the denizen permit** (`type(target) == "number"`, the Spirit Rend shape),
and TWO POSITIONS means the moment matters for the thing the whole bard basher is built around --
the back-position bonus (AB Footwork, doubled by Shadow Tempo). front->back is a straight win
(skip every front and side swing), back->side lands two hits from back where the natural cycle is
five away, and **side->front is the one loss** (two hits from back becomes five) -- so the pick
...
**Rimewrought / Famine (v4.7.333, user-directed).** *Rimewrought* ("Perpetual ice coats your body, rendering tattoos ineffective") is the Bravado shape for TATTOOS: `mnemRimewrought` (trigger `mnemosyne/096`) gates every tattoo we SEND on `ataxiaBasher_tattoosDead()` -- the danger-ladder shield, both flee fallbacks, the hostile-player exit, the class-cheese `touch shield`, the Psion `psi manipulate touch tree`, the blackout tree, and the lock-breaker's tree+shield -- and sends `curing tree off`, because the tree of life is a tattoo too (Splinterbark's flag and its run-end restore, reused). **Keep-up too (v4.7.334):** a defence on keep-up sits at `curing priority defence <def> 25`, a standing order SSC obeys forever, so it retried dead tattoos endlessly. `stripTattooKeepup()` resets exactly the tattoos the CURRENT profile asks for (read from `ataxiaTables.classDefences.tattoos`, so a new tattoo there is covered automatically) and `restoreTattooKeepup()` puts back exactly those on run end. **The saved profile is never edited** -- `ataxia.settings.defences` is on disk, and a config an affix edits stays edited after a bad session end; the priority is a game-side setting, like the tree curing beside it. *Famine* ("Taking damage has a chance to make you more hungry...") makes hunger a damage-driven cost, so `mnemFamine` (trigger `mnemosyne/097`) opens `ataxia_hornSatiate` with NO boon held and `ataxia_famineTopUp` runs on every room's last kill (trigger 340): corpse-first with Obligate Carnivore, horn otherwise, and the existing verify chain still stops at "utterly satiated". Both latch only in the tower, are transition-guarded, and clear on run end.
...
**OBLIGATE CARNIVORE + HEALING METABOLISM -- FOOD BECOMES UPKEEP (v4.7.294,
`ataxia_carnivoreEat`, `misc_scripts/022_Horn_Of_Plenty.lua`).** The horn exists because starvation
knocks you UNCONSCIOUS -- an emergency. Healing Metabolism ("health elixirs 50% more effective while
you possess the satiation defence") attacks the same axis from the other end: satiation stops being
a floor to stay above and becomes a DEFENCE WORTH HOLDING, and Obligate Carnivore ("you can EAT
corpses") supplies the food for free, delivered automatically since `340_Slain` ends "...retrieving
the corpse". **Corpses outrank the horn whenever the boon is held** -- a horn charge is one of six
on its own refill clock, a corpse is already in the pack. **Nothing is guessed**: the boon gives the
...
**DEATH STARE -- THE ONCE-PER-RIPPLE INSTANT KILL, COUNTED FROM THE KILL LINE (v4.7.306,
`ataxiaBasher_deathStare`, basher/001).** "CONSIDER now costs 3 seconds of equilibrium and
instantly kills any non-boss denizen target. This can only be used once per ripple." User: at 2+
denizens immediately, and at least once per ripple. A class-agnostic EQUILIBRIUM rider PREPENDED
to the assembled round in `ataxiaBasher_assembleAttack` (first in the chain so it lands on a LIVE
target; appended after the swing it could consider a corpse; for Magi it displaces one eq cast,
which a guaranteed kill is worth). Crowd first: at 2+ denizens it fires at once. Alone it holds --
but a charge that expires with the ripple must not go unused, so after
`ataxiaBasher.deathStareAfter` seconds (default 90, measured from the first round evaluated in
that ripple) it fires on whatever non-boss target is in front of us. **The charge is spent by the
kill line, not the send**: "<mob> freezes for a moment before dying. Instantly." (trigger
`mnemosyne/086`, captured live; self-proving, so it also re-latches the flag) marks it used and
releases the in-flight replay, so a send the server ate RETRIES after 8s instead of forfeiting the
ripple -- bounded at 3 tries so a missed line cannot cost an eq spend every few seconds all
ripple. The replay is bound to the TARGET it was sent for (a retarget mid-hold stops it rather than
considering a second mob). **The ripple key is the MAP's** (`MAP._ripple`, stamped by `M.onRipple`
unconditionally), not telemetry's `M.run.ripple`, which sits behind the `_auto()` gate and never
moves with reporting off. Never on the boss (`ataxiaBasher_targetIsBoss`, the legend deck's
first-word rule hoisted to a global) and never on a shielded target.
...
WEAKNESS row is parsed on the same shape with the noun swapped; that wording has never been seen,
so it is a place for the line to land, not a claim about it. **Unknown mobs are CONSIDERed once,
ever** (`ataxiaBasher_considerRecon`, from the round assembly): sent DIRECTLY, never in the
addclearfull chain, because CONSIDER is balanceless and a balanceless command in that chain
executes on every 0.3s rebuild (the shin-augment lesson); the row is created on the ATTEMPT so a
mob whose consider prints nothing we parse is still never asked twice (`bash resist forget <name>`
re-opens one); and NEVER while Death Stare is held, since that CONSIDER is the ripple's kill and
its output feeds the database anyway. Consumers ask `ataxiaBasher_targetResists(type)` /
`_targetWeakTo(type)` -- substring either way, so each asks in its own vocabulary, and an unknown
mob is never a guess. **Wired so far**: the Blademaster infuse element (`ataxiaBasher_bmInfuse`: a
known weakness outranks the preference order; a resisted element is skipped exactly as an
affix-suppressed one, with the same everything-suppressed fall-through) and the Bard's flick vs
punctuate (a mob that resists psychic gets punctuate, the fact the manual toggle existed for).
**Next, needing each attack's damage type confirmed from its AB before it can route**: Magi
staffcast (the crystal spells -- horripilation cold, immolation fire -- as an element choice),
Dragon gut/jab/whip/incantation vs the colour's blast, and the rest of the classes; the "Damage
dealt: N (type)" line trigger 350 already captures is the honest way to LEARN each attack's type
...
all denizens in your location." User: "in a room of 2 or more denizens the first thing we need to
...
replay, and BOTH lines captured live (trigger `mnemosyne/088`): the conjure ("You form a ball of
light in your palm and hurl it northwards.") releases the replay and restamps the room from the
LANDED moment; the boon's proc ("...searing the location with solar force.", an end fragment
because it wraps) prints only with the boon, so it also re-latches the flag. Fires on a shielded
round too -- the wall is room-wide, not a strike on the shield -- and still first, ahead of the
flay. **Open and deliberately not guessed**: whether our OWN lightwall darkshades US when the
sweep or the escape ladder walks through it; if it does, `SNUFF LIGHTWALLS` on room clear is the
fix and the explorer's room-clear hook is where it goes.
...
target health. **The AB and the boon disagree on three facts and each shapes the code**: AB Enfeeble
(Kaido 901) charges 61 kai (the boon removes it, so there is NO kai gate), says "Works on/against:
**Adventurers**" (the boon is the only thing making this legal in PvE, so it is `type(target) ==
"number"` gated -- against a player it is an ordinary 61-kai ability), and mentions no cooldown (the
60s is denizen-only). **No prone gate**: the AB's "reduced to 25% if not prone" clause is in the
ADVENTURER ability, and the boon restates the denizen effect flatly, so requiring prone would invent
a mechanic from the wrong paragraph. **No mana floor**, unlike Kai Choke, whose AB lists 50 mana --
this one lists kai and nothing else. **IT SORTS AHEAD OF KAI CHOKE**: three riders now share one eq
slot, resolved by an `or` chain (which short-circuits, so a loser is never CALLED and cannot stamp
its own retry guard for a round it did not win). Rend goes first because **its window CLOSES** --
illegal below 50%, and halving is worth twice as much at 80% as at 40% -- while the choke is just as
good next round; *the ability whose opportunity expires outranks the ones that merely recur*, the
Stormcleaver-before-Thunderclap reasoning. The 60s clock starts at the CONFIRMED line ("You
violently propel your kai energy at <mob>, enfeebling him.", trigger `080`, also chartreuse-bold
highlighted there rather than in a second trigger), never at send. An unreadable target does NOT
...
`_boonDbMerge`, which is FILL-ONLY -- it never overwrites a field the library already holds,
because observed data beats seeded data -- and every previous release had already merged the OLD
text in. `_bonusDesc` prefers the library over the seed, so the stale numbers would land on the
`mnem bonuses` panel as fact. Fill-only is right for ENRICHMENT and wrong for a RETCON, so
`M._boonSeedRetcon()` overwrites the named boons and drops the deleted ones **once per epoch**
(`M.BOON_SEED_EPOCH`, the `controlMobsSeeded`/`panicAt35` shape) -- once, because an unconditional
rewrite would undo a real in-game sighting on every load, which is the fill-only rule broken in the
other direction. `_learnBoon` DOES overwrite, so a sighting still wins afterwards; that
self-healing is not enough alone, since a boon may not come round for many runs and a RENAMED one
(`Trenchfoot` -> `Sprightly`) never comes round under the old name at all. **The 30 new boons whose
effects the announcement did not state are seeded as NAMES WITH NO DESCRIPTION** -- that is the
entry, not a gap: it makes the boon known so `mnem boondb` shows it as an explicit hole. They are
DECLARED in `M.BOON_UNDESCRIBED` rather than written inline, so the invariant got stricter -- the
suite asserts every undescribed entry is declared (an accidental omission still fails) and every
declaration is present (a stale one fails). Deliberately NOT extrapolated from the three Mastery
boons that do have text: one line would have generated the other five, which is exactly the
hand-maintained name->amount table this module exists to replace. `Cavalry` is on the "new" list
and we already had its text, so **"new" is not strictly new** and the `or {}` in the seeding loop
is load-bearing. **A BOON THAT CAPS HEALING BREAKS EVERY "STAND STILL UNTIL HEALTHY" GATE:**
`Battlemaster's Fury` used to read "you can no longer be healed above 30% health" against a
`recoverAt` of **95%**, so holding it made every hover and ground recovery burn its full 60s cap
forever -- the MANALEECH shape of v4.7.252 with a different cause, and nothing in the package knew.
It is a balance boon now, so there is nothing to fix; grep `recoverAt` when the next one appears.
Also: `Meathead`'s cost changed stupidity -> confusion, which is not cosmetic, because stupidity
EATS QUEUED COMMANDS. The 13 new affixes and their first-appearance ripples are recorded in
`.claude/projects/mnemosyne/03-parsing-triggers.md` rather than coded -- the affix layer learns
each sentence from WADE STATUS at runtime -- along with two facts that bound the worst case: the
enemy-spawning affixes are all mutually exclusive with one another (so `Necromantic`'s
repopulation, which is what makes `_roomHasDenizens` untrue, has a ceiling), and a manifested
nightmare's attack speed was reduced, which weakens the one seeded `controlMobs` default.

**`BOON CONTEMPLATE` NOW PRINTS A CATEGORY, AND THE PARSER WOULD HAVE EATEN IT (v4.7.288).**
`_parseContemplate` was a `meta -> desc -> quote` state machine that knew exactly three labels and
treated **any** other non-blank line as the start of the description -- so the new category line,
and the unlocked-by line, would have been concatenated into `info.description`, which reaches
`_learnBoon`, which OVERWRITES, replacing good library text with `"Category: Offensive Your fire
damage..."` and serving it to the bonuses panel. We have never seen the wording, so the fix matches
the SHAPE: while still in the meta block, a short `Label: value` line is meta, and an unrecognised
label is KEPT in `info.meta` rather than dropped -- storing it under whatever the game calls it is
how we learn what to call it. **The bound is the interesting half**: an unbounded shape rule
...
**SESSION STATS RESET ON A FRESH RUN, KEPT ACROSS A RESUME (v4.7.296).** The `tarc` HUD's
kills/gold/DPS/damage-taken now reset via `resetBashingStats()`, called from `M.onRunStart()`
right beside the identical AUDIT-baseline logic (v4.7.291) and for the same reason: a pause
(`WHISPER ... beseech that it grow still`) does not end the run server-side, so the next wade
re-enters the SAME run and wiping progress on a mere pause/resume would lose data for nothing.
Opposite of the ~40 boon flags reset UNCONDITIONALLY on the same trigger line -- those answer "do
we hold this boon right now" and a stale answer is actively wrong, where session stats carry no
such requirement. Above the `_auto()` gate, like the audit baseline, since session stats have no
dependency on REST telemetry.

**BERSERKER'S EDGE -- HOLD RAGE INSTEAD OF SPENDING IT (v4.7.296).** "1% extra damage per point of
battlerage held, up to 100 rage." Not a new system: `ataxiaBasher.rageFloor` (v4.7.141) already
exists for "gear pays a bonus while battlerage is at or above a threshold", and
`ataxiaBasher_rageAfford` already gates all 37 rotation call sites -- so the boon is that mechanism
PINNED AT ITS OWN CAP (`rageFloor = 100`), landing the hold on every class with zero per-rotation
code, since `rage >= cost + 100` can never hold for any real cost once rage itself caps at 100.
**Bypasses the user alias's `MAX_FLOOR` clamp (46) on purpose** -- that clamp exists so a human
typing a number cannot accidentally strand their own rotation, which is precisely the outcome this
automated boon wants. **The revert is designed with the effect** (the Borrowed Power rule,
v4.7.204): the pre-boon floor is saved ONCE (guarded against a second confirm overwriting it with
the boon's own 100) and restored on the confirmed run end, mirroring the Borrowed Power branch of
`M.onRunEnd()` exactly, plus a defensive revert at run start for a prior run that ended without
confirmation. **A manual `bash floor` command always wins**: it drops the saved-for-boon
...
INSIDE another timer's own callback -- an ordinary self-rearming pattern this codebase uses
throughout -- which adds a key to the table `pairs()` is still iterating, undefined in Lua and
surfaced as `invalid key to 'next'`. It fired only near a hash-table rehash boundary, so it was a
flake an unrelated test could trigger just by leaving a few more timers lying around than usual;
real Mudlet has no equivalent hazard, since its timer registry is not a Lua table this code
iterates. Fixed in both places by snapshotting keys before iterating; semantics otherwise
unchanged. **Break-back note:** the `fireOfferWait()` fix reproduced the crash 10/10 runs when
reverted; the general-purpose `mock.fire_timers()` fix, exercised only by a small dedicated test,
did not reproduce on 15 runs -- recorded honestly since forcing a specific rehash boundary
deterministically is not practical, not claimed as a clean break-back it did not achieve.
...
taking the first reports a penalty as a bonus; or TWO STATS SHARING ONE NUMBER). Ported in DESIGN
from a community window, and **three of the four porting-audit findings would each have shipped
broken**: `silver` (its Physical colour) IS NOT IN our palette and `fg()` throws at render time on a
panel that redraws per claim (hence `tools/check_colours.py`, below); its rarity palette uses the
RESERVED `orange` for epic (we use `gold`); and `Careless Whisperer` disagrees between the two
sources (community: Cold +10; our seed: an immunity boon) so it is **deliberately absent** and
contributes no row rather than a guessed one. Fire/Venom/Asphyxiation Mastery were missing from our
seed entirely and are added marked **SECOND-HAND**, the v4.7.278 honesty marker. Design calls:
**"All damage" folds into every TYPE** rather than a row of its own (at the point of use "+15% to
everything" and "+15% fire" are the same fact); **on-hit procs are gated on the literal phrase "your
attacks"**, because the identical "chance to afflict the target with" clause also covers
ABILITY-specific procs (draconic blast, `scream`, `confront`) that fire only on that one ability;
**type damage boosts stay OUT of the stat totals** (a Damage stat lifts everything, a Fire boost
lifts one type); **INERT is three-state** -- a Shaman boon whose spirit is unattuned is MARKED not
hidden, but `nil` means "cannot tell" and must never render as INERT; **COSTS is a section**, since
a panel showing only upside lies by arrangement. **NOT gated on `inMnem()`** (unlike the map): boons
persist for the whole run and the panel is as useful on the riverbank deciding whether to dive again
as it is mid-ripple. Bare `mnem bonuses` REPORTS to the console rather than toggling -- one source,
two surfaces -- and the empty panel says WHY it is empty, because a blank panel and a broken panel
look identical. 011 is pure and unit-tested (`test_mnem_bonuses.lua`); 012 is Geyser and is not, the
005/006 split exactly.
...
**A NAMESPACE UNDER `ataxia` IS PERSISTED WHETHER OR NOT YOU MEANT IT (v4.7.299).**
`ataxia_saveSettings` does `table.save(file, sanitizeForSave(ataxia))` -- WHOLESALE -- and
`sanitizeForSave` strips only FUNCTIONS, METATABLED objects and GUI snapshots. A table of ordinary
scalars goes straight to disk, and `deepMerge` ends in an unconditional `dst[k] = v` for non-tables,
so **the stored value WINS on load**. `ataxia.mnemosyne.run` was built on the belief that it was
"in-memory only" -- its header said so -- and `active`, `ripple`, `publicId`, `lastOffered`, `lives`
and `waveProgress` all came back from the previous session. **The telemetry was the least of it:**
...
while consumers that are NOT gated on reporting read it: `run.boss` makes the Bard pick WAVEDANCE
on an ordinary ripple and the legend deck WITHHOLD Xylthus (a card cannot bind a boss) and the
explorer patrol for a boss that is not there, and `run.ripple` drives the Bard's hawkstep gate and
the swarm's depth-scaled thresholds. `M._clearStaleRun()` fixes it, called from
`ataxia_loadSettings` -- **not** from a second `sysLoadEvent` handler, because ORDERING is the
subtle half: a separate handler's order relative to the loader is undefined, so it could run BEFORE
the merge and be undone by it, whereas inside the loader it is ordered by construction. `active`
and `boss` are cleared there but deliberately NOT folded into `_resetRun` (`startRun` sets `active`
true and then calls `_resetRun`; `boss` is re-learned from each ripple's Objective line), and a
test pins that so a future tidy-up cannot merge them. **Before parking any transient flag under
`ataxia`, decide whether it should be on `ataxiaTemp` instead** -- that is the same v4.7.192-194
rule, and this is the third system caught by it.

**THE FOURTH: THE TRACKER'S OWN HTTP QUEUE (v4.7.336).** `ataxia.mnemosyne._busy` and `_queue` sat
on the same saved namespace, beside the run state v4.7.299 fixed, and nobody looked. One save caught
a POST in flight; the next start restored `_busy = true` with no request on the wire and no watchdog
to clear it, so `_pump` returned early FOREVER and every report after it queued without a sound --
**711 requests, about nine runs**, in the live save, nothing sent, nothing failed, nothing echoed.
Each save wrote the wedge out again, so it survived every restart (and every XML reimport, which
reaches the same loader). Two general rules come out of it: **a busy flag restored without the timer
that clears it is a permanent livelock** -- a flag only a live timer can clear must never be
trusted after a reload -- and **never serialize a timer or event-handler id** (the v4.7.192 rule
again: `_watchdog` came back naming a stranger's timer, and the restored handler ids made the next
reinstall kill the wrong handlers and leave ours registered twice). `M._resumeQueue` (end of 001 AND
`ataxia_loadSettings` after the merge) clears the flag and REPLAYS the queue in order --
at-least-once, the user's choice over dropping it; the ids moved to `ataxiaTemp.mnemHttp`; `_pump`
refuses a busy flag with no send stamp from this session; and `mnem status` shows the queue, because
the only tell for nine runs was `ripple 0` on an active run.

**KNOWN DEFECT, UNFIXED: THE PROFILE-BACKUP RESTORE DESTROYS LIVE NAMESPACES (found 2026-09-09).**
...
`deepMerge`, `stripGui`, and the live-runtime-object guard whose ten-line comment sits directly
above it). Since `sanitizeForSave` strips FUNCTIONS, the backup's copy of a namespace is DATA ONLY
-- so assigning it over the live one **deletes that namespace's functions**: `ataxia.mnemosyne`
(the whole run tracker, map, explorer and swarm API), `ataxia.armour`, `ataxia.updater`,
`ataxia.bars`, `ataxia.defense`. It also re-introduces exactly the live-GUI-object corruption that
comment exists to prevent. Found because a test asserting the v4.7.299 reset runs on the backup
path FAILED -- the reset was unreachable, because the function it calls no longer existed by then.
The fix is to give that branch the file branch's merge semantics (hoist `deepMerge`/`stripGui` out
of `mergeLoad`); it changes disaster recovery for EVERY namespace under `ataxia`, so it wants its
own change and its own testing. **Until then a profile-backup restore needs a reload**, and the
`KNOWN DEFECT` comment at the site says so.
...
**BASE + PER-STACK, and a bare write is the BASE (announcement 2026-08-19, v4.7.278).** A bare
...
PvE bash set is active. Its classifier matched **letters only** until v4.7.278, so every
...
### Test files share one Lua state
`src_new/tests/test_runner.lua` runs every `test_*.lua` in ONE Lua state, in filename order, so a
global that a test file replaces (`send`, `io.open`, `tempTimer`, `getEpoch`, `ataxiaEcho`, a
module stub) is still replaced for every file after it. Restore it at the end of the file, and
inside a test restore it under `pcall` so a failing `expect` cannot leak it. Two real cases:
`test_settings.lua` mocked `io.open` (a write handle for any path opened without a mode) and never
restored it, so a later test reading a trigger file got a table with no `read` (fixed v4.7.321);
`test_swarm_tactics.lua` restores `send` part-way down the file, so blocks after that point capture
sends locally. A fixture that rewinds a mock clock must also clear every stamp taken on that clock,
or an earlier test's stamp sits in this test's future and reads as fresh.
...
**ARMY OF THE DEAD IS AN APOSTATE RIDER TOO (v4.7.347, user: "For Apostate").** `ataxiaBasher_infGravehands` has carried the Apostate command since v4.7.149 (`(class == "Apostate") and "summon hands of the grave" or "tyranny"`) and **nothing ever called it from the Apostate round** -- only `ataxiaBasher_infernalBashing` did -- so the boon was inert on this class: no error, no echo, simply nothing. Two differences from the Infernal copy, both from ABADMIN 144 (`SUMMON HANDS OF THE GRAVE`, 3.00s of EQUILIBRIUM, 1.50% life essence + 350 mana): (1) **it REPLACES the swing on its round, like TYRANNY** (corrected v4.7.354, user: "Gravehands takes balance") -- v4.7.347 believed the AB block's "3.00 seconds of EQUILIBRIUM" and prepended the summon beside the belch and the soulstorm, so the `deadeyes` swing behind it (also balance) was refused on every gravehands round; the belch and soulstorm (equilibrium) still ride that round; (2) **it costs 350 mana**, gated on the Apostate branch only (TYRANNY is a different ability and its cost is not this one) with a 40% floor -- lower than the belch's 50 because this is once-per-room, not every five seconds. The flat 350 check is NOT redundant with the floor: the floor is configurable and is inert when maxmp is unknown. **The once-per-room stamp is written at SEND time**, so a refused cast burns the room; the Apostate cast has a visible line (trigger 782 -> `ataxiaBasher_gravehandsUp`; matched on its FIRST-PERSON OPENING "You mutter words of death and decay..." since v4.7.351 -- the original fragment began at column 88 of a 148-character line and the server wrap went straight through it, so the confirmation never arrived and every room was cast twice), so an unconfirmed summon retries ONCE after `GRAVEHANDS_CONFIRM` 6s, per room. The Infernal is excluded from that retry by name -- TYRANNY's confirmation line was never captured, so "unconfirmed" would be its permanent state and it would re-cast every few seconds (the v4.7.148 essence burn). **That line does NOT latch the boon**, unlike the belch's: the base ability prints it with or without Army of the Dead, and a wrong latch costs 1.5% essence + 350 mana in every room after it -- the BOONS row (`mnemosyne/039`) and the claim line own that fact. Both lines are highlighted `cadet_blue` (belch goldenrod, soulstorm medium_orchid); the summon line WITHOUT the boon's "chill of the grave" line is the tell that the hands went up and the boon did not.

**GRAVEBORN -- gravehands in EVERY room (v4.7.348, user: "We need to ensure we gravehands every room to maximize this").** The combo boon (`mnemGraveborn`; rare / Offence / no echo; unlocked by **Army of the Dead + Maliceborn + Necrotic Aura**): *"While standing in gravehands, your attacks will command them to ravage your enemies, damaging all denizens in your location. This can only trigger every 15 seconds."* The crowd gate existed because the summon was ONE AoE hit; Graveborn makes the hands an ENGINE that fires every 15s for as long as we stand in them and swing, so a room we skip is an engine we never built. Three gates move with the boon, and all three had to, because any one of them silently skips a room: **denizens 2 -> 1**; **essence floor 20% -> 10%** (Graveborn is unlocked BY Maliceborn -- "slaying a denizen will now restore 5% of your life essence" -- so holding the combo means holding the refund); **mana floor 40% -> 25%**. LOWERED, NEVER WAIVED -- mana is the curing pool, and an explicit `infTyrannyAt`/`infEssenceFloor`/`gravehandsManaFloor` still beats the boon both ways. Wired in all five places a boon flag must be (BOON_FLAGS, run start, confirmed run end, claim line, BOONS row `mnemosyne/098`) plus the combo recipe. **NECROTIC AURA WAS INERT ON THE APOSTATE TOO** -- the identical v4.7.347 defect, found by reading what Graveborn is unlocked by: `ataxiaBasher_infDeathaura` is class-agnostic (DEATHAURA is a Necromancy defence) and only the Infernal round ever called it, so an Apostate on the combo path held a required component that did nothing. It now rides BOTH Apostate branches -- raising a defence is not an attack. **Two for two: when a helper is named for one class but its body is not, the name is not the gate, the CALL SITE is.**

**OUR BAALZADEEN WAS READING AS A SWARM (v4.7.348, user: "the denizens in the room should not count our Baalzadeen as Apostate! It is thinking it is a swarm").** `baalzadeen` and `falcon` have always been in the DEFAULT `ownDenizens` list -- which only runs when `ownDenizens` is nil, i.e. a fresh install -- while the backfill loop beside it, whose entire reason for existing is that "existing saves keep the old default", carried only `ashbeast` and `hyena`. So no existing save ever got them. The cost is not that the pet gets hit: `isOwnDenizen` is what `M._denizenCount`/`M._roomHasDenizens` filter through, so a missing keyword INFLATES THE SWARM COUNT, and that threshold fires the pull, the funnel and the icewall -- one pet beside two denizens reads as three, the default threshold exactly. Both are in the backfill now with a one-time echo. **A seeded default and a backfill are two different lists and only one runs for an existing user; adding to the seed alone ships a fix nobody receives.**

**PSION BOON KEEPERS (v4.7.349, user: "Psion is another class we need to add").** Two of the three pasted boons pay out only while a DEFENCE stands, which makes them keepers rather than attacks -- and ENACT costs EQUILIBRIUM (2.30s), which every Psion weave leaves idle, the same channel `psi transcend`/`enact wrath` already ride (the round sent `enact roth` -- the BOON's name -- until v4.7.360, so the heal never fired). `ataxiaBasher_psionEmulationKeepers(sp, rothFired)`: **Razor Clarity** (`psionRazorClarity`, +50% damage / +2% crit) keeps `clarity` via `enact clarity`; **Bloodletter's Fury** (`psionBloodletter`, weaves become unblockable, +50% base damage, 20% faster balance) keeps `rupturesight` via `enact rupture`. Commands and defence names are all confirmed, never guessed -- the two ENACTs are the package's own `emulation/004`/`005` aliases, both defence keys are already in `ataxiaTables.defences`, and the boon text names `rupturesight` itself. **THE HOLDS DIFFER DELIBERATELY** (clarity 12s, rupturesight 4s, `ataxiaBasher.psionKeepHold` overrides both): rupture's ability text is "your next three blows shall all deal significantly increased bleeding", which may make `rupturesight` a CHARGE rather than a lasting defence -- the keeper does not need to know, since it reads gmcp every round, so the uncertain one just gets the shorter anti-spam guard. They ride the shielded round too (raising a defence is not an attack). **ONE EQUILIBRIUM ACTION PER ROUND (v4.7.351):** roth, then psi transcend, then ONE keeper, RUPTURE FIRST -- v4.7.349 chained `enact clarity;enact rupture;`, and the second ENACT was refused while its hold was stamped as sent (the test asserted the collision as correct). The keepers take `eqSpent`, not just `rothFired`. Both keeper holds and the transcend hold are TIMESTAMPS (`ataxiaTemp.psionKeepAt[def]`, `psionTranscendAt`), not flags cleared by a tempTimer -- a lost timer wedged a keeper off for the session. **[REVERSED v4.7.356 -- shatter goes out ONLY at full transcendence, free, first in the round; user: "you keep using Psi shatter when we should be using weavings. Therefore, we should only use PSI Shatter when at 100 transcendence! PSI Shatter target;Weave deathblow target". The paid shatter below held every weave back: the queued round needs eq AND balance, and weaves build transcendence. What follows is v4.7.352's reasoning, kept for the record.]** **PSI SHATTER IS THE MAIN EQUILIBRIUM TOOL (v4.7.352, user: "Psi Shatter is the main tool to use unless we have other boons with full transcendence").** AB 2750: `PSI SHATTER <target>`, works on denizens, **3.10s of equilibrium** -- it was only ever sent at FULL transcendence (where it is free), and the user's log had it at 12,992 (Mindbreak) against deathblows of ~900-1,900. Equilibrium order: roth > psi transcend > ONE boon keeper > **psi shatter**; never shielded; rides a secondskin round. **At full transcendence the free shatter goes FIRST in the chain (v4.7.353)** -- it costs no equilibrium but REQUIRES equilibrium and balance to execute (user: "it does need EQ and Balance to execute but costs no EQ. Just requires it"; the wiki's "while off equilibrium" is wrong), so anything ahead of it that spent equilibrium would leave it none. With nothing else wanting equilibrium, the paid shatter followed it (v4.7.353-355; gone in v4.7.356 -- the round today is just `psi shatter <t>;weave deathblow <t>` at full transcendence and the weave otherwise). **PSIWAVE (v4.7.355, `psionPsiwave`)** swaps it for `psi radiate` (AB 2714, no target; with the boon it deals magic damage to every denizen) in `psionTranscendSpend(sp)` (v4.7.355 had a separate `psionEqAttack`, folded in v4.7.356) -- user: "When we have this boon please use this instead of shatter"; unconditional, over Mindbreak too. A keeper that fires now MARKS the round (`eqSpent`) -- before, it did not, which would have chained keeper + shatter (caught by the tests on the first run). The full-transcendence spend is `psionTranscendSpend(sp)` in basher/002 -- the one place a future boon changes it. **WRATH, NOT ROTH (v4.7.360).** The emergency heal is `ENACT WRATH` (AB 2739, 1.30s eq, below 50% HP, every 3 minutes, grants clarity + rupture); the round sent `enact roth` -- the Mnemosyne boon's name -- from the start, so it never fired, and every test asserted the wrong command. The **Roth** combo boon (Razor Clarity + Bloodletter's Fury, `psionRoth`) makes it 75% HP on a 30s cooldown. Lockouts (user, v4.7.361): 120s normally, 30s with Roth. **Rupture (v4.7.361)** is tracked from the game's lines -- up/already (psion/007) stand the Bloodletter keeper down, the down line (psion/008, "Distractions reassert themselves...") clears the belief and the hold so the next round re-enacts; the belief expires after 20s. Wrath is still stamped at BUILD -- a replaced round loses it for the lockout -- until its game line is captured. **Mindbreak changes no rotation** -- it multiplies the one free shatter per full transcendence. **EARTHQUAKE (v4.7.357, `psionEarthquake`)**: `enact upheaval` (AB 2730, 2.30s eq; "deals significant blunt damage to all denizens in the location when it summons rubble") rides on the equilibrium nothing else took -- after roth/transcend/keepers, after the free shatter, beside the weave -- via `ataxiaBasher_psionUpheaval(sp, eqSpent)`; only with 2+ denizens (`ataxiaBasher.upheavalAt`), at 50%+ HP, unshielded, because its rubble blocks OUR exits too. Costs a deathblow round 0.10s. Leaving over rubble is a slow clamber, so `M.onClamber()` (008, trigger mnemosyne/104) re-arms the explorer's pending move timeout for `CLAMBER_GRACE` (10s) once per move -- the 5s timeout used to re-send the step and could condemn a real exit. **PROPHET OF CREATION (v4.7.358, `psionProphet`)** -- foresight works on denizens and makes the next attack against us miss -- was latched only in v4.7.358 and is USED since v4.7.359: `ataxiaBasher_psionForesight(sp)` opens the round with `psi foresight <target>` (user: "No tree or shield that is optional") whenever it is off cooldown and the target is at 50%+ HP -- a foresight that does not come true stuns us, and against a denizen that means a target that dies before it swings. The cooldown is stamped by the CAST LINE (psion/005) and the refusal retries in 5s (psion/006); the build only takes a 2s in-flight hold, because a rebuilt round replaces the queued one and a build-time 30s stamp would lose the dodge. Free per the user; a test pins that the round is unchanged by it. First in-game test: the prediction is simply the denizen's next attack ("Your prediction comes to pass, and you effortlessly avoid the attack from <denizen>."), true in under 0.3s, no stun; about a 30s cooldown (refusal: "Your mind has not yet recovered enough to pierce the fabric of time once again."); the open question is a denizen that dies or never swings. **The round is re-queued the moment transcendence fills** (`ataxiaBasher_requeueNow`, genrunning/004, called from `psion/002` only on the change to 100): the round is rebuilt on prompt/vitals events behind a 0.3s anti-spam flag, so the "achieved transcendence" line could land after the last rebuild and the stale entry fired without the free shatter -- the user's "behind one attack". It clears that flag for one call and goes through tryAttack's normal gates; with no target it does nothing.

**THE TRANSCENDENCE DECAY LINE COULD NEVER HAVE MATCHED (v4.7.349).** `psion/001_Transcendence_Set` watched both directions with `$`-anchored patterns. The decay line ("Your inaction causes the harmonisation of your body and mind to falter: you are now only N percent of the way to transcendence.") is **128 characters**; Achaea wraps server-side at ~114, so it arrives in halves and an anchored pattern cannot match it. The BUILD line is 94, fits, and matched fine -- which is what hid it: **transcendence counted up and never came back down**, so `ataxiaBasher_psionBashing` kept sending `psi shatter <target>` believing it was at 100 when the game had taken it away. Both patterns now stop at the captured number (keeping only `^`), and "Your body and mind are no longer in harmony." -- the bottom of the ladder, previously unwired -- zeroes the counter in trigger 003. **The fix created a display bug (v4.7.351):** `deleteFull()` gags the first row and only deletes the next if it is a prompt, so the tail "transcendence." was left alone on screen every tick; `psion/004` gags it, but only within a second of `psion/001` firing (`ataxiaTemp.transcendLineAt`). **An anchored pattern is a bet that the line is short enough, and the lines that break it are exactly the informative ones** (v4.7.286 reached the same lesson from the other side: there the answer was a fragment, here it is stopping before the wrap).

**A RELOAD NOW BRINGS EVERY BOON BACK (v4.7.350).** Boon flags are bare globals, so a reimport clears them; `M._relatchBoons` sends `boon claimed` once per run so the rows (`<name>  <echoes>  <rarity>`) can re-latch what we own. `M.BOON_FLAGS` documented itself as latched from "the BOON CLAIM ... and the BOONS list (on demand, and after a reload)" -- **only the claim half was ever wired**. The generic row trigger `mnemosyne/013_Boons_List_Row` learned, recorded, coloured and annotated every row and never called `latchBoonFlag`, so a boon returned after a reload only if it ALSO had a hand-written row trigger. 15 of the table's 20 did not -- including Dead Breath and Deathtempest, so a mid-run reimport silently stopped the Apostate's belch and soulstorm (and Dead Breath cannot self-rescue from its boon line, because the belch that prints it never fires without the flag). The row trigger now calls `latchBoonFlag(name)` -- **only while `ataxiaBasher.inMnemosyne`** (v4.7.351: only one of the twenty consumers, Sharp Mind, checks for itself, so a list printed outside a run would have armed the belch and soulstorm for ordinary bashing; the wade-status trigger sets the flag before the re-latch is sent, and the per-boon rows 098-101 carry the same gate). **Run start now calls `M.clearBoonFlags()` too** (v4.7.351) -- it had cleared only a hand-written list missing 13 of the 20. Safe because the pattern requires an echo count AND a rarity word -- the offer screen's `Name:   description` rows have neither. **When adding a boon to `BOON_FLAGS`, no row trigger is needed any more** -- a test feeds a row for every name in the table and requires every flag back. Panoply (the user's prompt for this) was never affected: it has its own row trigger, and `weave flurry` has replaced `weave deathblow` under it since it shipped.

**A TRIGGER PATTERN MUST FIT ON ONE WRAPPED ROW (v4.7.351).** This user's Achaea wraps server-side at **119-124 columns** (measured from the break points in their pastes), so a long line reaches Mudlet as separate rows and a pattern that runs past the break -- or a fragment that straddles it -- can never match, silently. Three triggers from one week did exactly that (782 gravehands confirmation, 779 belch fouled, 780 soulstorm landing for long mob names) plus the highlights for all three. **Match an EARLY phrase of any line near or over ~110 characters; for a highlight, add a short anchored tail `^.{0,60}<last words>\.$` so the wrapped row is coloured too.** Chat pastes arrive UNWRAPPED, so a paste is not evidence of the line's length. Two guards: `tools/check_wrap.py` (CI) fails any pattern whose shortest match is wider than 118 columns (8 pre-existing allowlisted; a stale entry fails the check), and `tests/test_trigger_wrap.lua` + `tests/trigger_lib.lua` catch the straddling class the lint cannot see -- they wrap known lines at 100/119/124 and run the trigger's REAL patterns per row, through an evaluator that raises on any regex shape it cannot judge. **Assert on the PATTERN, never on the file text** -- after the first fix, 782's old test still passed on the comment explaining the fix.

**MOUNTJUMP RECOVERS BOTH WAYS (v4.7.351).** "You have no mount on which to jump." (705) now re-sends a recorded MOUNTJUMP as `leap` (`ataxiaBasher_jumpRefusedOnFoot`) -- a stale mounted belief (died in the saddle, mount killed, thrown by an uncaptured line) previously lost the escape. Both recoveries HOLD the send, still latching, while a tumble is in flight (`S.moveLocked`) or we stand in lava (`M.onLava` owns movement). `S.reset` clears the jump ledger; `ataxiaBasher_onDeath` sets the belief to UNKNOWN (nil) above its enabled-check. **The gravehands once-per-room record is cleared at each ripple** (`M.onRippleReset`) -- the tower reuses room numbers.
...
### A guarded start with an unguarded stop is not a pair (v4.7.292)

The balance banner printed `BALANCE: 0.000` constantly. **Two triggers stop one stopwatch** --
`balances/001_Limb_Balance` ("...on your legs.") and `balances/006_All_Limbs` ("...on all limbs.")
both call `endBalTimer()` + `balanceHighlight()`, and both lines arrive in the same round. The first
stopped and RESET the watch; the second stopped an already-reset watch, read ~0, and printed it as a
measurement. `timerOnBalUsed` had always refused to start a running watch; nothing ever refused to
stop a stopped one.

Two rules generalise:
- **Guard both ends or neither.** EQ was corrected at the same time despite having only one stopper:
  the asymmetry was the defect, and fixing only the half that bites leaves the other waiting for a
  second trigger to be added.
- **"Zero" and "nothing was running" are different answers.** The fabricated 0.000 was written into
  `bashStats.lastBalanceTime` and zeroed `currentBalanceDamage` a second time, so a round's damage
  was attributed to a balance of no length -- a display bug that was quietly corrupting DPS.
