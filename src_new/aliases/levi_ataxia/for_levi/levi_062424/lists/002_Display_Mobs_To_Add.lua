...
if ataxia.denizensHere and ataxiaBasher.targetList[gmcp.Room.Info.area] then
	...
	for i,v in pairs(ataxia.denizensHere) do
		if not table.contains(ataxiaBasher.targetList[gmcp.Room.Info.area], v) and not table.contains(ataxiaBasher.mobIgnore, v) and not ataxiaBasher_isOwnDenizen(v) then
			cecho("\n  <green>+ ")
			fg("white")
