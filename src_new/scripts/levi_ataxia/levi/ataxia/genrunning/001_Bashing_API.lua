...
-- Kill-before-register so a SYSUPDATE reload doesn't stack duplicate handlers (this file
-- ALSO used to contain a verbatim second copy of itself, registering everything twice —
-- deduped 2026-07-24; the registry below prevents any recurrence of double-fired events).
ataxiaBasherApi_handlers = ataxiaBasherApi_handlers or {}
local function _reg(event, fnName)
  if ataxiaBasherApi_handlers[event] then pcall(killAnonymousEventHandler, ataxiaBasherApi_handlers[event]) end
  ataxiaBasherApi_handlers[event] = registerAnonymousEventHandler(event, fnName)
end
_reg("mmapper failed path", "ataxiaBasher_pathFail")
_reg("mmapper arrived", "ataxiaBasher_arrived")
_reg("player death", "ataxiaBasher_onDeath")
_reg("attacker class detected", "ataxiaBasher_onAttacked")
