...
-- Guard: gmcp.Room / gmcp.Room.Info can be nil during transitions (login, boon
-- screen, GO!/wade, between-room hops in Mnemosyne). The prompt trigger calls
-- search_targets() every prompt, so an unguarded index spams the error console.
local function hasRoomInfo()
  return gmcp and gmcp.Room and gmcp.Room.Info ~= nil
end
