...
function shikudoRiftLock.status()
  ...
  cecho("\n<cyan>╔══════════════════════════════════════════════╗")
  cecho("\n<cyan>║         <white>SHIKUDO RIFTLOCK STATUS<cyan>             ║")
  cecho("\n<cyan>╠══════════════════════════════════════════════╣")

  cecho("\n<cyan>║ <white>Phase: " .. phaseColor .. phase)
  cecho("\n<cyan>║ <white>Form: <yellow>" .. form .. " <grey>(" .. kata .. "/" .. maxKata .. " kata)")
  cecho("\n<cyan>║ <white>Mindlock: " .. (mindlocked and "<green>ACTIVE" or "<red>INACTIVE"))
  cecho("\n<cyan>║ <white>Blackout Active: " .. (shikudoRiftLock.state.blackoutActive and "<red>YES - OPPONENT BLIND" or "<grey>No"))

  cecho("\n<cyan>╠══════════════════════════════════════════════╣")
  cecho("\n<cyan>║ <white>BURST READINESS:")
  cecho("\n<cyan>║   <white>In Rain: " .. (form == "Rain" and "<green>YES" or "<red>NO"))
  cecho("\n<cyan>║   <white>Kata >= 9: " .. (kata >= 9 and "<green>YES (" .. kata .. ")" or "<yellow>NO (" .. kata .. "/9)"))
  cecho("\n<cyan>║   <white>Can Blackout: " .. (shikudoRiftLock.canBlackout() and "<green>YES" or "<red>NO (cooldown)"))
  cecho("\n<cyan>║   <white>BURST READY: " .. (shikudoRiftLock.isReadyForBurst() and shikudoRiftLock.canBlackout() and "<green>*** YES ***" or "<yellow>NO"))

  cecho("\n<cyan>╠══════════════════════════════════════════════╣")
  cecho("\n<cyan>║ <white>HINDRANCE AFFLICTIONS:")
  cecho("\n<cyan>║   " .. (tAffs.paralysis and "<green>[X]" or "<red>[ ]") .. " paralysis <grey>(stops parry)")
  cecho("\n<cyan>║   " .. (tAffs.clumsiness and "<green>[X]" or "<red>[ ]") .. " clumsiness <grey>(miss attacks)")
  cecho("\n<cyan>║   " .. (tAffs.weariness and "<green>[X]" or "<red>[ ]") .. " weariness <grey>(blocks Fitness)")
  cecho("\n<cyan>║   " .. (tAffs.lethargy and "<green>[X]" or "<dim_grey>[ ]") .. " lethargy")

  cecho("\n<cyan>╠══════════════════════════════════════════════╣")
  cecho("\n<cyan>║ <white>LOCK AFFLICTIONS:")
  ...
  cecho("\n<cyan>║     " .. (tAffs.impatience and "<green>[X]" or "<red>[ ]") .. " impatience <grey>(Telepathy)")

  -- Truelock
  local truelock = shikudoRiftLock.checkTruelock()
  cecho("\n<cyan>║   <white>Truelock: " .. (truelock and "<green>[LOCKED]" or (hard and "<yellow>[NEXT]" or "<dim_grey>[PENDING]")))
  cecho("\n<cyan>║     " .. (tAffs.weariness and "<green>[X]" or "<red>[ ]") .. " weariness")

  cecho("\n<cyan>╠══════════════════════════════════════════════╣")
  cecho("\n<cyan>║ <white>LIMB DAMAGE:")
  cecho("\n<cyan>║   <white>L Arm: <yellow>" .. string.format("%.1f%%", shikudoRiftLock.getArmDamage("left")))
  cecho("  <white>R Arm: <yellow>" .. string.format("%.1f%%", shikudoRiftLock.getArmDamage("right")))
  cecho("\n<cyan>║   <white>L Leg: <yellow>" .. string.format("%.1f%%", shikudoRiftLock.getLegDamage("left")))
