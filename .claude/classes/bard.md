...
mechanic:
  position_tracking: "bardtempo (front/side/back) is set by tempo/001-003 off the game's position lines. UNTIL v4.7.313 THOSE PATTERNS TOOK `\\w+` FOR THE PARTNER'S NAME -- a player's one word, never a denizen's 'a royal guard of Zanzibaar' -- so in PvE the position was NEVER updated: Shadow Tempo's back-bonus rule (v4.7.241) never saw 'back', and the footwork flourish, seeing a 'front' that never changed, flourished on every balance in a live log (back -> side -> front -> back, no attack ever landing). Now `.+?`. The pick also latches ONCE PER VISIT TO THE FRONT, released only when the position is seen to leave it, so a stuck flag costs one balance rather than every balance."
...
config:
  ...
  footworkFlourish: "ataxia.bardStuff.footworkFlourish (default TRUE since v4.7.312, user: 'We should flourish from the front to get to the back'; a v4.7.311 save's false is migrated once). 'bashflourish on|off|always': BLADE FLOURISH at every return to FRONT (front -> back), boon or no boon. ON applies the tempo rule -- Vivace always, Adagio/Moderato only with Shadow Tempo, never Allegro/none -- read from the GAME's tempo line; ALWAYS drops the rule (every return to front, whatever the tempo); OFF leaves flourish to the boon (ataxiaBasher_bardFootworkFlourishPays, basher/002)."
...
| "all denizens in your location" | boon | **2+ denizens** (v4.7.313, user: "only use flourish with multiple denizens and the boon, or in the front stance to get to the back stance" -- the later instruction; v4.7.302 had removed the gate at the user's request). The footwork policy covers the lone mob |
