...
psi_shatter: "ONLY at full transcendence, free, first in the round (v4.7.356) -- see 'The round today' below"
...
eq_riders: |
  ...
  (no paid shatter since v4.7.356 -- shatter goes out free at full transcendence only)
...
2. **At most one equilibrium-spending action**, the first that applies: `enact wrath` (below 50% HP; 75% and every ~35s with Roth)
...
PSI Shatter target;Weave deathblow target"*). v4.7.352-355 also spent idle equilibrium on a PAID
shatter (AB 2750: 3.10s of equilibrium) -- but the queued round needs equilibrium AND balance, so
that held every weave back to shatter's equilibrium, and weaves are what build transcendence. The
full-transcendence spend lives in `psionTranscendSpend(sp)` (basher/002): shatter, or radiate with
Psiwave.
...
| Bloodletter's Fury | `psionBloodletter` | keep rupture up (`enact rupture`) "every time it is available": weaves unblockable, +50% base, 20% faster balance. Since v4.7.361 the keeper believes the game's lines -- up "Your vision sharpens, allowing you to perceive the locations of every vein...", already "Your blows will already rupture veins and arteries." (psion/007), down "Distractions reassert themselves, your mental clarity returning to mundane levels." (psion/008; reads like clarity's, but the user confirmed "It is rupture dropping") -- down clears the hold so the next round re-enacts; the belief expires after 20s. All three are highlighted bold `deep_pink` (highlighting/067, v4.7.364) |
...
AB text is "your next four blows", so it reads like a charge and gets the shorter guard). A keeper that fires marks the round's
...
| `PSI TRANSCEND` / `PSI SHATTER` | Weaves charge transcendence; at FULL transcendence `psi shatter <t>` goes out free, first in the round (costs no eq but REQUIRES it), and ONLY then (v4.7.356 -- a paid shatter every round held the weaves back) |
