...
| `mnem boonfill unknown` | Names BOON CONTEMPLATE refused. **Only a name we never saw offered lands here** (v4.7.328): a refusal for a name the game printed on the screen means the screen closed -- you claimed before the chain reached it -- and is not recorded. Says once how many entries the v4.7.328 repair cleared |
...
| `mnem combos [filter]` | Boon COMBO recipes (v4.7.328): reward, its components (green = held this run), and progress, closest to done first. Learnt from the REWARD's own `Unlocked By` line; components we have never contemplated ride along with the offer chain, one per screen. Says so when no run is being tracked (progress comes from the run's claims, so nothing counts as held between runs), and flags a reward you hold but never learnt |
