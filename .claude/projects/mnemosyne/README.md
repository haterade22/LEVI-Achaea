...
| `mnemosyne/004_Parsers.lua` | Game-text parsers: block capture (force-finishes a wedged prior capture instead of dropping the new one), run pause/resume, `_extractMob`, monster capture, immediate boon-offer posting + enrichment |
...
| `mnemosyne/008_Explorer.lua` | Auto-sweep the 4×4 (drives the basher in manual mode); pauses at the boon screen with the basher left on, auto-resumes on `GO!` |

Script paths relative to `src_new/scripts/levi_ataxia/levi/ataxia/`. Triggers live in `src_new/triggers/levi_ataxia/for_levi/leviticus/mnemosyne/` (`001`–`016`, incl. `011_Ice_Slip` and `016_Run_Pause`); aliases in `src_new/aliases/levi_ataxia/for_levi/levi_062424/mnemosyne/` (`001`–`002`).
...
Config lives in `ataxia.settings.reporting` (`enabled`, `contemplate`, `token`, `url`, `debug`, `mapEnabled`, `quiet`) — saved inside the main `ataxia` settings file via `ataxia_saveSettings()`, no new disk file. **Run state is in-memory only** (`ataxia.mnemosyne.run`); on load it re-syncs via `/run_exists` (the server is the source of truth). That was aspirational until v4.7.299 — the table sits under `ataxia`, which is saved wholesale, so it really did come back from disk; `M._clearStaleRun()` (called from `ataxia_loadSettings`) is what makes the sentence true. The **local history** (`ataxia.mnemosyne.history`) is the one thing persisted to its own file, `<profile>/mnemosyne_history.lua` (see [06-history.md](06-history.md)).
