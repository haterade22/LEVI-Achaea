...
| 105 | Prophet of Creation | `^Prophet of Creation\s+\d+\s+\w+` | sets `psionProphet` (in the tower only) -- `psi foresight <target>` opens the Psion round when off cooldown and the target is at 50%+ HP (v4.7.359; latch-only in v4.7.358) |
| 106 | Roth | `^Roth\s+\d+\s+\w+` | sets `psionRoth` (in the tower only) -- `enact wrath` below 75% HP on a 35s lockout (v4.7.360) |
