...
| 029 | Haemophiliac | `^Haemophiliac:\s+Defeating a denizen causes you to bleed` (status-screen effect row) | `onHaemophiliacSeen()` — arms wade-slower pacing (post-clear explorer moves hold until HP >= 90%, `M._haemoHold`) | inline `inMnemosyne` gate + transition guard (mirrors Splinterbark) |
