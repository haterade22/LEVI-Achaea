...
| **Auto-Learn** | Adds room denizens to the area target list on entry; `bash mine` excludes your own pets/allies (falcons, your Baalzadeen, etc.) without skipping the room -- and they no longer inflate the swarm count |
| **Your Mounts** | `bash mounts` -- mounts are skipped as targets by their ID (a wild dire wolf is still fair game); VAULT makes that mount your active one; while mounted every tactical jump is a MOUNTJUMP, and a refused jump in either direction is re-sent with the right verb |
...
| `armour <name>` | Swap to a named profile (e.g., `armour bash`, `armour pvp`): probes the armour and `ii paragon` first, swaps only the embrasures that change, inserts the paragon of that type you hold (by its id), changes nothing if the game does not answer the probe, reports any paragon you don't have (that embrasure is left alone, never emptied), then probes again to verify |
...
A passthrough intercept of `BOON CLAIM <name>` forwards your command, auto-reports the selection, and flips combat flags so the basher adapts to the boons you take — Bard **Warmarch**, Blademaster **Shattered Star** / **Bladed Reflexes** (keeps the Shindo augment up for 20% damage reduction), and Magi **Aspect of Kkractle** / **Hot Springs** reshape their class's attack logic, and the class-agnostic **Hammer and Anvil** (attacks bypass denizen shields) turns off razing and shield-swapping entirely for the rest of the run. **Bloodscent** turns each ripple entry into parsed recon (below). Boons are **called out**: the offer screen notes each boon that is a **combo** boon, how many times it can **echo**, and what it **conflicts** with (red for a boon you already hold, yellow for another on the same screen); `BOON CONTEMPLATE` highlights the same lines and adds a one-line summary. After every offer, the **boon advisor** summarises the options -- what each does, anything wrong with it (already immune, a conflict, a drawback) -- and **recommends one**, defence first and judged against the boons you already hold (`mnem advise` re-prints it; it never claims for you). `mnem boondb` shows and filters by **category** (`mnem boondb offence`). **Boon combos** are tracked too: a set of boons unlocks another one, **24 recipes ship known** (from contemplate blocks players pasted), and `mnem combos` shows each recipe, which parts you hold and how far along you are — the advisor counts a boon that carries a combo forward (one chain's worth, even if it feeds several), scores a boon far lower when its cost is an affliction you cannot block — and back up again once you hold that immunity, the components are contemplated one per offer screen until the recipe is filled in, and the granted reward is recorded, flagged and reported when the mist bestows it. Monk/Shikudo gets two more: **Kai Unleashed** fires a room-wide KAI CHOKE burst (25k+ magical observed) alongside the combo in crowded rooms on its 30-second cooldown — confirmed by the burst's own game line, so an eaten choke never wastes the window — and **Senseless Flurry** keeps the numbness defence up in thin rooms for 30% faster balance (crowd-gated: numbness defers damage and would blind the safety nets in a swarm). Any Monk holding **Sharp Mind** (critical strikes restore mana, and the Monk strikes three times a balance) transmutes mana into health as a top-up at 90% HP rather than a 70% gap-filler. Psion's **Panoply** swaps the bash to WEAVE FLURRY (60–200% scaling per strike); **Bloodletter's Fury** and **Razor Clarity** keep the rupturesight and clarity defences up on idle equilibrium, and every full transcendence is cashed in for a free **PSI SHATTER** ahead of the weave -- or, with **Psiwave**, a **PSI RADIATE** that hits every denizen in the room. The weaves are the rotation; shatter is the reward for keeping them going. The Psion's emergency heal is ENACT WRATH below half health -- or, with the **Roth** combo boon (Razor Clarity + Bloodletter's Fury), below 75% every ~30 seconds. **Prophet of Creation** opens the round with a free PSI FORESIGHT on the target every ~30 seconds, so its next attack misses (only while the target is at half health or more -- a foresight that never comes true stuns you). **Earthquake** adds ENACT UPHEAVAL on spare equilibrium in a crowd above half health (its rubble blocks your own exits, so the explorer waits out the clamber instead of calling the move failed). Apostate necromancy boons: **Dead Breath** belches at 2+ denizens and **Deathtempest** soulstorms each denizen once, both riding the round on equilibrium beside the deadeyes swing; **Army of the Dead** summons gravehands once per room, taking that round's balance in place of the swing; **Graveborn** makes that every room, and **Necrotic Aura** keeps DEATHAURA up. The **Rimewrought** affix takes tattoo defences off keep-up until it clears; **Famine** eats to full every room. After a reimport mid-run, the first ripple re-latches every boon you hold from `boon claimed`. The **Haemophiliac** affix (kills bleed thousands) automatically slows the sweep: after each room clears, movement holds until the bleed is clotted and HP recovers. **Boss tactics** are per-boss scripted: against Seasone the Industrious the tree tattoo is held in reserve from the moment her objective appears and released the instant her phial truelock lands — a fresh tree breaks the lock immediately.
